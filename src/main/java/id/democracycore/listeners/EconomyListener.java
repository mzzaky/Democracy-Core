...
public class EconomyListener implements Listener {
    ...
    public double applyVaultBonus(Player player, double amount) {
        ...
        if (gov.hasPresident() && gov.getPresidentUUID().equals(player.getUniqueId())) {
            bonus += plugin.getConfig().getDouble("president.buffs.vault-bonus", 0.20);
        }
        ...
        if (treasuryMinister != null && treasuryMinister.equals(player.getUniqueId())) {
            bonus += plugin.getConfig().getDouble("cabinet.positions.treasury.buffs.vault-bonus", 0.15);
        }
    ...
    public void processTransaction(Player from, Player to, double amount, String reason) {
        ...
        if (tax > 0) {
            plugin.getTreasuryManager().deposit(Treasury.TransactionType.TAX_INCOME, tax, "Transaction tax: " + reason, null);
        }
    ...
    public double getXpMultiplier(Player player) {
        ...
        // President XP bonus
        if (gov.hasPresident() && gov.getPresidentUUID().equals(player.getUniqueId())) {
            multiplier += plugin.getConfig().getDouble("president.buffs.xp-bonus", 0.10);
        }
        ...
        if (cultureMinister != null && cultureMinister.equals(player.getUniqueId())) {
            multiplier += plugin.getConfig().getDouble("cabinet.positions.culture.buffs.xp-bonus", 0.10);
        }
    ...
    public double getDropMultiplier(Player player) {
        ...
        if (plugin.getExecutiveOrderManager().isOrderActive(ExecutiveOrder.ExecutiveOrderType.ENVIRONMENTAL_PROTECTION)) {
