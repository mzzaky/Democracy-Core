...
public class BuffManager {
    ...
    public void applyPresidentBuffs(Player player) {
        ...
        double damageBonus = plugin.getConfig().getDouble("president.buffs.damage-bonus", 0.15);
        double defenseBonus = plugin.getConfig().getDouble("president.buffs.defense-bonus", 0.12);
        int extraHearts = plugin.getConfig().getInt("president.buffs.extra-hearts", 2);
    ...
    public void applyCabinetBuffs(Player player, Government.CabinetPosition position) {
        if (player == null || !player.isOnline() || position == null) return;
        ...
        String basePath = "cabinet.positions." + position.name().toLowerCase() + ".buffs.";
        
        double damageBonus = plugin.getConfig().getDouble(basePath + "damage-bonus", 0);
        double defenseBonus = plugin.getConfig().getDouble(basePath + "defense-bonus", 0);
        int extraHearts = plugin.getConfig().getInt(basePath + "extra-hearts", 0);
    ...
    private void applyCabinetPotionEffects(Player player, Government.CabinetPosition position) {
        switch (position) {
            ...
            case CULTURE -> {
                ...
                player.addPotionEffect(new PotionEffect(PotionEffectType.DOLPHINS_GRACE, Integer.MAX_VALUE, 0, false, false, true));
            }
        }
    }
    ...
    // Get buff descriptions for display
    public String getPresidentBuffDescription() {
        double damageBonus = plugin.getConfig().getDouble("president.buffs.damage-bonus", 0.15) * 100;
        double defenseBonus = plugin.getConfig().getDouble("president.buffs.defense-bonus", 0.12) * 100;
        int extraHearts = plugin.getConfig().getInt("president.buffs.extra-hearts", 2);
        double vaultBonus = plugin.getConfig().getDouble("president.buffs.vault-bonus", 0.20) * 100;
        double xpBonus = plugin.getConfig().getDouble("president.buffs.xp-bonus", 0.10) * 100;
