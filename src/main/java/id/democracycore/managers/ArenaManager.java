...
public class ArenaManager {
    ...
    // Leaderboard rewards
    private static final int[] DAILY_REWARDS = {100000, 75000, 60000, 50000, 40000, 35000, 30000, 27500, 25000, 25000};
    ...
    public boolean canStartArena() {
        ...
        if (gamesThisTerm >= maxGames) return false;
        
        // Check cost
        int cost = plugin.getConfig().getInt("arena.start-cost", 100000);
        return plugin.getDataManager().getTreasury().getBalance() >= cost;
    }
    
    public boolean startArena(UUID presidentId) {
        if (!canStartArena()) return false;
        
        Government gov = plugin.getDataManager().getGovernment();
        if (!gov.getPresidentUUID().equals(presidentId)) return false;
        
        // Deduct cost
        int cost = plugin.getConfig().getInt("arena.start-cost", 100000);
        plugin.getTreasuryManager().withdraw(Treasury.TransactionType.PRESIDENTIAL_GAMES, (double) cost, "Presidential Arena Games", presidentId);
        
        // Create new session
        long durationDays = plugin.getConfig().getLong("arena.duration-days", 7);
        currentSession = new ArenaSession(presidentId, durationDays * 86400000L);
        ...
        arenaRadius = plugin.getConfig().getInt("arena.radius", 100);
    ...
    public void endArena() {
        ...
        for (int i = 0; i < topPlayers.size(); i++) {
            ...
            MessageUtils.broadcast("<yellow>" + medals[i] + " " + name + " - " + entry.getValue().getKills() + " kills");
        }
    ...
    public boolean leaveArena(UUID playerId) {
        if (!arenaPlayers.contains(playerId)) return false;
    ...
    public void handleKill(Player killer, Player victim) {
        ...
        if (!arenaPlayers.contains(killer.getUniqueId()) || !arenaPlayers.contains(victim.getUniqueId())) return;
        ...
        // Base reward
        int baseReward = plugin.getConfig().getInt("arena.kill-reward", 5000);
        ...
        if (killerData != null) killerData.incrementKillstreak();
        if (victimData != null) victimData.resetKillstreak();
        
        plugin.getDataManager().saveAll();
    }
    
    public void handleDeath(Player player) {
        if (!arenaPlayers.contains(player.getUniqueId())) return;
    ...
    public void distributeDailyRewards() {
        ...
        for (int i = 0; i < leaderboard.size(); i++) {
            ...
            if (player != null && player.isOnline()) {
                ...
                MessageUtils.send(player, "<gold>Daily arena reward: $" + MessageUtils.formatNumber(reward) + " + " + netherite + " netherite!");
            } else {
            ...
            MessageUtils.broadcast("<yellow>" + medal + " <white>" + name + " <gray>- " + kills + " kills <green>($" + MessageUtils.formatNumber(reward) + ")");
        }
    ...
    private void distributeFinalRewards() {
        ...
        if (finalStandings.isEmpty()) return;
        ...
        MessageUtils.broadcast("<gold>🏆 <yellow>Arena Champion: <white>" + winnerName + " <gray>(" + winner.getValue().getKills() + " total kills)");
    }
